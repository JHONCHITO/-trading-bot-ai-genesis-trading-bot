...
function pnl(position: Position, exitPrice: number, config: BotConfig) {
  ...
  const gross = position.plan.side === "buy" ? (exitPrice - entry) * position.plan.units : (entry - exitPrice) * position.plan.units;
  return gross - commission;
}

export class Backtester {
  constructor(private readonly config: BotConfig, private readonly bot: TradingBot) {}

  async run(candles: Candle[]) {
    const state: PortfolioState = { equity: this.config.initialCapital, peakEquity: this.config.initialCapital, sessionPnl: 0, cooldownBars: 0, openPositions: 0, halted: false };
    ...
    for (let i = lookback; i < candles.length; i += 1) {
      ...
      if (state.cooldownBars > 0) state.cooldownBars -= 1;
      ...
      if (openPosition) {
        ...
        if (exit) {
          ...
          if (tradePnl < 0) state.cooldownBars = this.config.cooldownBarsAfterLoss;
      ...
      if (state.equity <= this.config.initialCapital * (1 - this.config.maxDrawdownPct)) state.halted = true;
      ...
      if (!openPosition && !state.halted) {
        ...
        if ((decision.action === "enter-long" || decision.action === "enter-short") && decision.plan) {
          ...
          openPosition = {
            ...
            entryFill,
            entryIndex: i,
            features: decision.signal?.features ?? { trend: 0, flow: 0, breakout: 0, volume: 0, volatility: 0, spread: 0 },
    ...
    const std = trades.length > 1 ? Math.sqrt(trades.reduce((sum, t) => sum + (t.returnPct - avgReturn) ** 2, 0) / (trades.length - 1)) : 0;
